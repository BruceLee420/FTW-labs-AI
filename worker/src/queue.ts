...
/**
 * The storefront adapter.
 *
 * DELIBERATELY NOT IMPLEMENTED. At the time of writing the connected Shopify
 * store returns `operation_not_allowed` — "This shop is unavailable for API
 * access. The merchant may need to resolve a billing issue or upgrade their
 * plan." With the store unreachable there is no way to observe a real
 * request/response for productCreate, and writing a guessed mutation shape
 * would mean the first live run is also the first test.
 *
 * Everything upstream of this function is real and working: the queue, the
 * edits, the holds, the timer. This is the single stub, and PUBLISH_ENABLED
 * keeps it dark until it is genuinely wired.
 *
 * To finish it: restore store API access, read the ProductInput schema, build
 * the mutation, verify against a draft product, then implement here and flip
 * the flag.
 */
export async function publishToStore(
  _row: QueueRow,
  _env: { PUBLISH_ENABLED?: string },
): Promise<{ ok: false; error: string } | { ok: true; externalId: string; externalUrl: string }> {
  return {
    ok: false,
    error:
      "Storefront publishing is not wired yet. Shopify API access is unavailable for this store " +
      "(operation_not_allowed), so the mutation shape could not be verified. See queue.ts:publishToStore.",
  };
}

/** Cron entry point. Walks due rows, re-checks holds, publishes or defers. */
export async function runAutoDeploy(db: D1Database, env: { PUBLISH_ENABLED?: string }): Promise<void> {
  ...
  for (const row of due) {
    ...
    if (holds.length > 0) {
      await db
        ...
        .run();
      await logEvent(db, row.id, "held", holds.join(" "), "system");
      continue;
    }
    ...
    if (env.PUBLISH_ENABLED !== "true") {
      await db
        .prepare("UPDATE queue SET status = 'held', hold_reasons = ?, publish_after = NULL, updated_at = ? WHERE id = ?")
        .bind(JSON.stringify(["Publishing is disabled (PUBLISH_ENABLED is not 'true')."]), now(), row.id)
        .run();
      await logEvent(db, row.id, "held", "Publishing disabled", "system");
      continue;
    }
    ...
    if (result.ok) {
      await db
        .prepare(
          `UPDATE queue SET status = 'published', external_id = ?, external_url = ?,
             published_at = ?, updated_at = ?, last_error = NULL WHERE id = ?`,
        )
        .bind(result.externalId, result.externalUrl, t, t, row.id)
        .run();
      await logEvent(db, row.id, "published", result.externalUrl, "system");
    } else {
      const attempts = row.attempts + 1;
      // Give up rather than retry forever — a stuck row that keeps hammering
      // the store is worse than one that waits for a human.
      ...
      await db
        ...
        .run();
      await logEvent(db, row.id, status === "failed" ? "failed" : "publish_error", result.error, "system");
    }
  }
}
